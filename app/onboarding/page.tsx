  ...
  return (
    <button
      ...
      style={{
        ...
        background: selected ? 'rgba(196,18,48,0.06)' : 'transparent',
        color: selected ? '#C41230' : 'rgba(0,0,0,0.4)',
        cursor: 'pointer',
        fontFamily: 'var(--font-geist-mono, monospace)',
...
export default function OnboardingPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F8F8F7', color: '#0A0A0A', fontFamily: 'var(--font-geist-mono, monospace)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 24px' }}>
      {/* Logo */}
      ...
      <div style={{ width: '100%', maxWidth: 640 }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
            <span style={{ fontSize: 9, letterSpacing: '0.12em', color: 'rgba(0,0,0,0.35)' }}>STEP {step} OF {TOTAL_STEPS} — {stepTitles[step - 1]}</span>
            <span style={{ fontSize: 9, letterSpacing: '0.12em', color: 'rgba(0,0,0,0.25)' }}>{Math.round((step / TOTAL_STEPS) * 100)}%</span>
          </div>
          <div style={{ height: 2, background: 'rgba(0,0,0,0.08)' }}>
            <div style={{ height: '100%', width: `${(step / TOTAL_STEPS) * 100}%`, background: '#C41230', transition: 'width 0.3s' }} />
          </div>
        </div>

        {/* Card */}
        <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', padding: '36px' }}>

          {/* Step 1 */}
          {step === 1 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div>
                <label style={labelStyle}>COMPANY NAME <span style={{ color: 'rgba(196,18,48,0.6)' }}>*</span></label>
              ...
              <div>
                <label style={labelStyle}>UEI NUMBER <span style={{ color: 'rgba(0,0,0,0.2)' }}>(OPTIONAL)</span></label>
              ...
              <div>
                <label style={labelStyle}>WEBSITE <span style={{ color: 'rgba(0,0,0,0.2)' }}>(OPTIONAL)</span></label>
              ...
              <div>
                <label style={labelStyle}>YEAR FOUNDED <span style={{ color: 'rgba(0,0,0,0.2)' }}>(OPTIONAL)</span></label>
          ...
          {step === 2 && (
            <div>
              <div style={{ fontSize: 10, color: 'rgba(0,0,0,0.4)', marginBottom: 20, lineHeight: 1.6 }}>Select all that apply to your business.</div>
          ...
          {step === 3 && (
            <div>
              <div style={{ fontSize: 10, color: 'rgba(0,0,0,0.4)', marginBottom: 20, lineHeight: 1.6 }}>Select all NAICS codes that apply to your business.</div>
              ...
              {form.naicsCodes.length > 0 && (
                <div style={{ marginTop: 16, fontSize: 10, color: '#C41230', letterSpacing: '0.08em' }}>
          ...
          {step === 4 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
              <div>
                <div style={{ fontSize: 9, letterSpacing: '0.16em', color: 'rgba(0,0,0,0.25)', marginBottom: 14 }}>CONTRACT SIZE</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {['Micro (<$10K)', 'Simplified ($10K-$250K)', 'Large ($250K+)', 'Any'].map((size) => (
                    <div key={size} onClick={() => setField('contractSizePref', size)} style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                      <div style={{ width: 14, height: 14, borderRadius: '50%', border: form.contractSizePref === size ? '2px solid #C41230' : '2px solid rgba(0,0,0,0.15)', background: form.contractSizePref === size ? '#C41230' : 'transparent', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {form.contractSizePref === size && <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#ffffff' }} />}
                      </div>
                      <span style={{ fontSize: 12, color: form.contractSizePref === size ? '#C41230' : 'rgba(0,0,0,0.45)', fontFamily: 'var(--font-geist-sans, sans-serif)' }}>{size}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div>
                <div style={{ fontSize: 9, letterSpacing: '0.16em', color: 'rgba(0,0,0,0.25)', marginBottom: 14 }}>CONTRACT TYPE</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[
                  ...
                  <div key={key} onClick={() => setField(key, !form[key])} style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                    <div style={{ width: 14, height: 14, border: form[key] ? '1px solid #C41230' : '1px solid rgba(0,0,0,0.15)', background: form[key] ? '#C41230' : 'transparent', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      {form[key] && <span style={{ fontSize: 9, color: '#ffffff', fontWeight: 700 }}>✓</span>}
                    </div>
                    <span style={{ fontSize: 12, color: form[key] ? '#C41230' : 'rgba(0,0,0,0.45)', fontFamily: 'var(--font-geist-sans, sans-serif)' }}>{label}</span>
                  </div>
                ))}
              </div>
              <div>
                <div style={{ fontSize: 9, letterSpacing: '0.16em', color: 'rgba(0,0,0,0.25)', marginBottom: 12 }}>SPECIFIC STATES</div>
          ...
          {step === 6 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ fontSize: 10, color: 'rgba(0,0,0,0.4)', marginBottom: 8, lineHeight: 1.6 }}>Select all certifications and clearance levels your company holds.</div>
              {CERTIFICATIONS.map((cert) => (
                <div key={cert} onClick={() => setField('certifications', toggle(form.certifications, cert))} style={{ display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer' }}>
                  <div style={{ width: 14, height: 14, border: form.certifications.includes(cert) ? '1px solid #C41230' : '1px solid rgba(0,0,0,0.15)', background: form.certifications.includes(cert) ? '#C41230' : 'transparent', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {form.certifications.includes(cert) && <span style={{ fontSize: 9, color: '#ffffff', fontWeight: 700 }}>✓</span>}
                  </div>
                  <span style={{ fontSize: 12, color: form.certifications.includes(cert) ? '#C41230' : 'rgba(0,0,0,0.45)', fontFamily: 'var(--font-geist-sans, sans-serif)' }}>{cert}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {step === 7 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              {/* Summary */}
              <div style={{ background: '#F8F8F7', border: '1px solid rgba(0,0,0,0.07)', padding: '20px' }}>
                <div style={{ fontSize: 9, letterSpacing: '0.16em', color: 'rgba(0,0,0,0.25)', marginBottom: 16 }}>PROFILE SUMMARY</div>
                {[
                  ...
                  <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: 10, marginBottom: 10, borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
                    <span style={{ fontSize: 9, letterSpacing: '0.1em', color: 'rgba(0,0,0,0.3)' }}>{item.label}</span>
                    <span style={{ fontSize: 11, color: '#0A0A0A', textAlign: 'right', maxWidth: '60%', fontFamily: 'var(--font-geist-sans, sans-serif)' }}>{item.value}</span>
                  </div>
                ))}
              </div>
              ...
              {/* Plan selection */}
              <div>
                <div style={{ fontSize: 9, letterSpacing: '0.16em', color: 'rgba(0,0,0,0.25)', marginBottom: 14 }}>CHOOSE YOUR PLAN</div>
                  ...
                  {PLANS.map((plan) => {
                    ...
                    return (
                      <div
                        ...
                        style={{ background: selected ? 'rgba(196,18,48,0.03)' : '#F8F8F7', border: '1px solid rgba(0,0,0,0.08)', padding: '16px 20px', cursor: 'pointer', borderLeft: selected ? '2px solid #C41230' : '2px solid transparent' }}
                      >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            <div style={{ width: 12, height: 12, borderRadius: '50%', border: selected ? '2px solid #C41230' : '2px solid rgba(0,0,0,0.15)', background: selected ? '#C41230' : 'transparent' }} />
                            <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', color: selected ? '#C41230' : 'rgba(0,0,0,0.6)' }}>{plan.name}</span>
                            {plan.popular && <span style={{ fontSize: 8, letterSpacing: '0.1em', color: '#C41230', padding: '2px 6px', border: '1px solid rgba(196,18,48,0.25)' }}>POPULAR</span>}
                          </div>
                          <span style={{ fontSize: 12, fontWeight: 700, color: selected ? '#C41230' : 'rgba(0,0,0,0.4)', fontFamily: 'var(--font-geist-sans, sans-serif)' }}>{plan.price}</span>
                        </div>
                        <ul style={{ listStyle: 'none', padding: '0 0 0 22px', margin: 0, display: 'flex', flexWrap: 'wrap', gap: '4px 16px' }}>
                          {plan.features.map((f) => (
                            <li key={f} style={{ fontSize: 10, color: 'rgba(0,0,0,0.35)', display: 'flex', gap: 6, fontFamily: 'var(--font-geist-sans, sans-serif)' }}>
                              <span style={{ color: '#C41230' }}>—</span>{f}
                            </li>
                          ))}
                        </ul>
                      </div>
          ...
          {/* Navigation */}
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 36, paddingTop: 24, borderTop: '1px solid rgba(0,0,0,0.07)' }}>
            <button
              type="button"
              onClick={() => setStep((s) => Math.max(1, s - 1))}
              disabled={step === 1}
              style={{ padding: '10px 20px', fontSize: 10, letterSpacing: '0.1em', background: 'transparent', border: '1px solid rgba(0,0,0,0.1)', color: 'rgba(0,0,0,0.35)', cursor: step === 1 ? 'not-allowed' : 'pointer', opacity: step === 1 ? 0.3 : 1, fontFamily: 'var(--font-geist-mono, monospace)' }}
            >
              ← BACK
            </button>
            ...
            {step < TOTAL_STEPS ? (
              <button
                ...
                style={{ padding: '10px 24px', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', background: '#C41230', color: '#ffffff', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-geist-mono, monospace)' }}
              >
                CONTINUE →
              </button>
            ) : (
              <button
                ...
                disabled={loading}
                style={{ padding: '10px 24px', fontSize: 10, fontWeight: 700, letterSpacing: '0.1em', background: '#C41230', color: '#ffffff', border: 'none', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.6 : 1, fontFamily: 'var(--font-geist-mono, monospace)' }}
              >
