  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0A', padding: '48px clamp(20px, 5vw, 64px)', fontFamily: mono }}>
      <div style={{ maxWidth: 860, margin: '0 auto' }}>
        ...
        {(['company', 'founder'] as const).map(aud => {
          const group = posts.filter(p => p.audience === aud)
          if (group.length === 0) return null
          return (
            <div key={aud} style={{ marginBottom: 44 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 6, flexWrap: 'wrap' }}>
                <span style={{ color: crimson, fontSize: 10, fontWeight: 700, letterSpacing: '0.16em' }}>
                  {aud === 'company' ? 'IR COMPANY PAGE' : 'YOUR PROFILE'}
                </span>
                <span style={{ color: 'rgba(255,255,255,0.25)', fontSize: 11 }}>{group.length} drafts</span>
              </div>
              <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 12.5, lineHeight: 1.7, margin: '0 0 18px', maxWidth: 600, fontFamily: sans }}>
                {aud === 'company'
                  ? 'Plural and institutional, but written like a person runs it. Data forward, willing to hold an opinion.'
                  : 'First person and opinionated. People follow people — this should never sound like the company page wrote it.'}
              </p>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {group.map(p => {
                  const imgUrl = p.image
                    ? `/api/post-image?stat=${encodeURIComponent(p.image.stat)}&label=${encodeURIComponent(p.image.label)}&sub=${encodeURIComponent(p.image.sub)}`
                    : null
                  return (
                    <div key={p.kind} style={{ border: '1px solid rgba(255,255,255,0.09)', background: '#111' }}>
                      ...
                      {imgUrl && (
                        <div style={{ padding: '0 20px 18px' }}>
                          ...
                          <img src={imgUrl} alt="" width={260} height={260} style={{ border: '1px solid rgba(255,255,255,0.1)', display: 'block' }} />
                          <a
                            href={imgUrl}
                            download={`ir-${p.kind}.png`}
                            style={{ display: 'inline-block', marginTop: 10, padding: '7px 14px', fontSize: 9, fontWeight: 700, letterSpacing: '0.1em', color: '#fff', background: crimson, textDecoration: 'none' }}
                          >
                            DOWNLOAD PNG
                          </a>
                        </div>
