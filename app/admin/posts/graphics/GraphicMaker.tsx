...
export interface Preset { name: string; stat: string; label: string; sub: string }

// Fallback only. Real presets arrive from the server, computed from the
// contract store — a hardcoded number would be the exact thing this page
// exists to avoid putting in front of people.
...
export default function GraphicMaker({ presets }: { presets?: Preset[] }) {
  const list = presets && presets.length > 0 ? presets : FALLBACK
  const first = list[0]
  const [stat, setStat] = useState(first.stat)
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(260px, 1fr) minmax(240px, 340px)', gap: 24, alignItems: 'start' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div>
          <label style={lbl}>PRESETS · FROM LIVE DATA</label>
        ...
        <div style={{ padding: '14px 16px', border: '1px solid rgba(196,18,48,0.3)', background: 'rgba(196,18,48,0.05)' }}>
          ...
          <p style={{ fontSize: 12, lineHeight: 1.65, color: 'rgba(255,255,255,0.5)', margin: 0, fontFamily: sans }}>
            Presets are computed from the contract store, so they are already true. If you type your
            own number, it becomes a public claim under your name — phrase market volume as what IR
            tracked, since the store is a synced subset of SAM.gov rather than a complete census.
          </p>
        </div>
      </div>
